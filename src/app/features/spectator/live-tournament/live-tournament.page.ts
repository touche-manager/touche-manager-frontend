...
export class LiveTournamentPageComponent implements OnInit, OnDestroy {
  ...
  readonly bracketRoundData = computed<BracketRoundData[]>(() => {
    ...
    const keys = (Object.keys(b.roundBouts) as EliminationRound[]).sort((a, c) =>
      LiveTournamentPageComponent.ROUND_ORDER.indexOf(a) -
      LiveTournamentPageComponent.ROUND_ORDER.indexOf(c)
    );
      ...
      bouts: (b.roundBouts[round] || [])
        ...
        .map((bout): BracketCardData => {
          ...
          return {
            ...
            refereeLabel: bout.referees?.length ? bout.referees[0].fullName : null,
            status: bout.status
          };
        })
    }));
